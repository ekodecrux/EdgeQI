...
const PROJECT_COLORS = ['#5B6CFF', '#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899'];
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ fontSize: 11 }}>
        ...
        <div style={{ color: '#94A3B8' }}>last: {last.toFixed(0)}%</div>
      </div>
    </div>
  );
}
...
export default function ProjectHub({ currentProjectId, onSelectProject, onNavigateTo }: ProjectHubProps) {
  ...
  // Form states
  const [form, setForm] = useState({ name: '', description: '', app_url: '', tech_stack: '', owner_email: '', color: '#5B6CFF', icon: '🚀', status: 'active' });
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setForm({ name: '', description: '', app_url: '', tech_stack: '', owner_email: '', color: '#5B6CFF', icon: '🚀', status: 'active' });
        if (created?.id) {
  ...
  return (
    <div style={{ fontFamily: '"Inter", Arial, sans-serif' }}>
      ...
      {/* ── PAGE HEADER ── */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 20, borderBottom: '1px solid #E2E8F0', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg,#0F172A 0%,#5B6CFF 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FolderOpen style={{ width: 22, height: 22, color: '#fff' }} />
          </div>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0F172A', margin: 0 }}>Project Hub</h1>
            <p style={{ fontSize: 13, color: '#475569', margin: '2px 0 0' }}>Create, manage and track all your applications end-to-end through STLC</p>
          </div>
        </div>
        <button onClick={() => { setEditingProject(null); setJustCreatedProject(null); setForm({ name: '', description: '', app_url: '', tech_stack: '', owner_email: '', color: '#5B6CFF', icon: '🚀', status: 'active' }); setShowCreateProject(true); }}
          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', background: '#5B6CFF', color: '#fff', border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 2px 8px #5B6CFF40' }}>
      ...
      {justCreatedProject && (
        <div style={{ background: 'linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%)', border: '1.5px solid #86efac', borderRadius: 14, padding: '18px 24px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#166534' }}>
                ✅ Project "<span style={{ color: '#0F172A' }}>{justCreatedProject.name}</span>" created!
              </div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {[
              { label: '📋 Add Requirements', tab: 'requirements', color: '#5B6CFF' },
            ...
            <button onClick={() => setJustCreatedProject(null)}
              style={{ width: 28, height: 28, borderRadius: 7, border: '1px solid #bbf7d0', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569' }}>
      ...
      {activeProject && !justCreatedProject && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: activeProject.color + '0e', border: `1px solid ${activeProject.color}35`, borderRadius: 12, padding: '12px 18px', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#0F172A' }}>Active: {activeProject.name}</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 1 }}>{activeProject.tech_stack || activeProject.description || 'No details set'}</div>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, flexWrap: 'wrap' }}>
        {(['projects', 'sprints', 'runs'] as const).map(v => (
          <button key={v} onClick={() => setActiveView(v)}
            style={{ padding: '8px 20px', borderRadius: 8, fontSize: 13, fontWeight: activeView === v ? 700 : 500, border: `1px solid ${activeView === v ? '#5B6CFF' : '#E2E8F0'}`, background: activeView === v ? '#eaf5fd' : '#f8fafc', color: activeView === v ? '#5B6CFF' : '#475569', cursor: 'pointer', textTransform: 'capitalize' }}>
            {v === 'projects' ? `🗂 Projects (${projects.length})` : v === 'sprints' ? '🏃 Sprints' : '▶ Run History'}
          </button>
        ))}
        {activeView !== 'projects' && (
          <select value={filterProject} onChange={e => setFilterProject(e.target.value)}
            style={{ marginLeft: 'auto', border: '1px solid #E2E8F0', borderRadius: 8, padding: '7px 12px', fontSize: 12, color: '#0F172A', background: '#fff', cursor: 'pointer' }}>
      ...
      {loading && <div style={{ textAlign: 'center', padding: '40px', color: '#475569', fontSize: 13 }}>Loading projects…</div>}
      ...
      {!loading && activeView === 'projects' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 18 }}>
          {projects.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: '#fff', border: `2px solid ${isActive ? p.color : '#E2E8F0'}`, borderRadius: 14, padding: 20, transition: 'all 0.18s', boxShadow: isActive ? `0 4px 20px ${p.color}25` : '0 1px 4px rgba(31,57,101,0.06)', display: 'flex', flexDirection: 'column', gap: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: '#0F172A', marginBottom: 3 }}>{p.name}</div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => startEdit(p)} title="Edit project"
                      style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid #E2E8F0', background: '#f8fafc', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {/* Description */}
                <p style={{ fontSize: 12, color: '#475569', marginBottom: 12, lineHeight: 1.55, minHeight: 34 }}>{p.description || 'No description added yet'}</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: '#f8fafc', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px', textAlign: 'center' }}>
                ...
                {/* App URL */}
                {p.app_url && (
                  <div style={{ fontSize: 11, color: '#5B6CFF', marginBottom: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                ...
                <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <button onClick={() => openProject(p)}
                    style={{
                      ...
                      background: isActive
                        ...
                        : `linear-gradient(135deg, #0F172A 0%, #5B6CFF 100%)`,
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7 }}>
                    <button onClick={() => { onSelectProject(p.id); onNavigateTo('testcases'); }}
                      style={{ padding: '7px 6px', background: '#f8fafc', border: '1px solid #E2E8F0', borderRadius: 7, fontSize: 11, fontWeight: 600, color: '#475569', cursor: 'pointer', textAlign: 'center' }}>
                      🔬 Test Cases
                    </button>
          ...
          <button onClick={() => { setEditingProject(null); setJustCreatedProject(null); setForm({ name: '', description: '', app_url: '', tech_stack: '', owner_email: '', color: '#5B6CFF', icon: '🚀', status: 'active' }); setShowCreateProject(true); }}
            style={{ background: '#f8fafc', border: '2px dashed #E2E8F0', borderRadius: 14, padding: 20, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, minHeight: 220, transition: 'all 0.18s' }}>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: '#eaf5fd', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Plus style={{ width: 26, height: 26, color: '#5B6CFF' }} />
            </div>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#5B6CFF' }}>Create New Project</div>
      ...
      {!loading && activeView === 'sprints' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {filteredSprints.length === 0 && (
            <div style={{ textAlign: 'center', padding: '50px', color: '#475569', background: '#f8fafc', borderRadius: 12, border: '1px dashed #E2E8F0' }}>
          ...
          {filteredSprints.map(s => {
            ...
            return (
              <div key={s.id} style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 4px rgba(31,57,101,0.04)' }}>
                {/* Sprint row */}
                <div style={{ display: 'flex', alignItems: 'center', padding: '14px 18px', cursor: 'pointer', gap: 14 }} onClick={() => setExpandedSprint(isExpanded ? null : s.id)}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#475569', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.goal || 'No sprint goal set'}</div>
                  </div>

                  {/* Velocity sparkline */}
                  <div style={{ flexShrink: 0, minWidth: 160 }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Pass Rate Trend</div>
                    <VelocitySparkline values={velocityData} color={proj?.color || '#5B6CFF'} />
                  </div>

                  {/* Meta */}
                  <div style={{ display: 'flex', gap: 14, fontSize: 12, color: '#475569', flexShrink: 0 }}>
                    ...
                    <span style={{ color: '#94A3B8' }}>▶ {sprintRuns.length} runs</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
                    {['active', 'completed'].filter(st => st !== s.status).map(st => (
                      <button key={st} onClick={e => { e.stopPropagation(); updateSprintStatus(s.id, st); }}
                        style={{ fontSize: 10, padding: '4px 9px', borderRadius: 6, border: '1px solid #E2E8F0', background: '#f8fafc', color: '#0F172A', cursor: 'pointer', fontWeight: 600 }}>
                        → {st}
                      </button>
                    ))}
                  </div>
                  {isExpanded ? <ChevronDown style={{ width: 16, height: 16, color: '#475569', flexShrink: 0 }} /> : <ChevronRight style={{ width: 16, height: 16, color: '#475569', flexShrink: 0 }} />}
                </div>
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid #f1f5f9', padding: '16px 18px', background: '#f8fafc' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                      ...
                      {proj && (
                        <button onClick={() => { onSelectProject(proj.id); onNavigateTo('execution'); }}
                          style={{ padding: '6px 12px', background: '#fff', border: '1px solid #E2E8F0', borderRadius: 7, fontSize: 11, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>
                          ▶ Run Execution
                        </button>
                      )}
                    </div>
                    ...
                    {sprintRuns.length === 0 ? (
                      <div style={{ fontSize: 12, color: '#94A3B8', textAlign: 'center', padding: '20px' }}>No runs recorded in this sprint yet</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                        {[...sprintRuns].reverse().map((r, idx) => (
                          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, padding: '10px 14px' }}>
                            <div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 700, width: 20, textAlign: 'right', flexShrink: 0 }}>#{sprintRuns.length - idx}</div>
                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: r.pass_rate >= 90 ? '#10b981' : r.pass_rate >= 70 ? '#f59e0b' : '#ef4444', flexShrink: 0 }} />
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 700, color: '#0F172A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.run_label || r.id}</div>
                              <div style={{ fontSize: 11, color: '#475569' }}>{r.module} · {r.environment} · {r.branch}</div>
                            </div>
                            <div style={{ display: 'flex', gap: 10, fontSize: 11, color: '#475569', flexShrink: 0 }}>
                            ...
                            <div style={{ fontSize: 10, color: '#94A3B8', flexShrink: 0 }}>{new Date(r.created_at).toLocaleDateString()}</div>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              {filteredRuns.length === 0 && (
                <tr><td colSpan={11} style={{ textAlign: 'center', padding: '40px', color: '#94A3B8' }}>No runs recorded yet. Runs are created automatically when you execute tests.</td></tr>
              )}
              {filteredRuns.map((r, i) => {
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafbfd' }}>
                    <td style={{ padding: '10px 12px', fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontWeight: 700, color: '#5B6CFF', fontSize: 11 }}>{r.id}</td>
                    <td style={{ padding: '10px 12px' }}>{proj ? <span>{proj.icon} {proj.name}</span> : <span style={{ color: '#94A3B8' }}>—</span>}</td>
                    <td style={{ padding: '10px 12px', color: '#475569' }}>{sprint?.name || '—'}</td>
                    <td style={{ padding: '10px 12px' }}><span style={{ background: '#eaf5fd', color: '#5B6CFF', padding: '2px 8px', borderRadius: 20, fontWeight: 600, fontSize: 10 }}>{r.module}</span></td>
                    <td style={{ padding: '10px 12px', color: '#475569', textTransform: 'capitalize' }}>{r.run_type}</td>
                    ...
                    <td style={{ padding: '10px 12px', color: '#475569' }}>{r.duration_ms > 0 ? `${(r.duration_ms / 1000).toFixed(1)}s` : '—'}</td>
                    <td style={{ padding: '10px 12px', color: '#475569' }}>{r.environment}</td>
                    <td style={{ padding: '10px 12px', color: '#94A3B8', whiteSpace: 'nowrap' }}>{new Date(r.created_at).toLocaleDateString()}</td>
                    <td style={{ padding: '10px 12px', color: '#475569', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.notes || '—'}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 560, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(31,57,101,0.25)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              ...
              <button onClick={() => { setShowCreateProject(false); setEditingProject(null); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569' }}>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Project Icon</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {PROJECT_ICONS.map(ic => (
                  <button key={ic} onClick={() => setForm(f => ({ ...f, icon: ic }))}
                    style={{ width: 38, height: 38, borderRadius: 9, border: `2px solid ${form.icon === ic ? '#5B6CFF' : '#E2E8F0'}`, background: form.icon === ic ? '#eaf5fd' : '#f8fafc', cursor: 'pointer', fontSize: 18, transition: 'all 0.12s' }}>
                    {ic}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Brand Color</label>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                {PROJECT_COLORS.map(c => (
                  <button key={c} onClick={() => setForm(f => ({ ...f, color: c }))}
                    style={{ width: 28, height: 28, borderRadius: '50%', background: c, border: form.color === c ? '3px solid #0F172A' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.12s' }} />
                ))}
                {/* Preview */}
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Description</label>
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="What does this application do? What's the testing scope?" rows={3}
                style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }} />
            </div>
            ...
            <div style={{ marginBottom: 22 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Project Status</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {['active', 'planning', 'archived'].map(s => (
                  <button key={s} onClick={() => setForm(f => ({ ...f, status: s }))}
                    style={{ padding: '6px 16px', borderRadius: 20, fontSize: 11, fontWeight: 700, textTransform: 'capitalize', border: `1px solid ${form.status === s ? '#5B6CFF' : '#E2E8F0'}`, background: form.status === s ? '#eaf5fd' : '#f8fafc', color: form.status === s ? '#5B6CFF' : '#475569', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={saveProject} disabled={saving || !form.name.trim()}
                style={{ flex: 1, padding: '12px', background: saving || !form.name.trim() ? '#94A3B8' : '#5B6CFF', color: '#fff', border: 'none', borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: saving || !form.name.trim() ? 'not-allowed' : 'pointer' }}>
                {saving ? '⏳ Saving…' : editingProject ? '✓ Save Changes' : '🚀 Create Project'}
              </button>
              <button onClick={() => { setShowCreateProject(false); setEditingProject(null); }}
                style={{ padding: '12px 22px', background: '#f8fafc', color: '#475569', border: '1px solid #E2E8F0', borderRadius: 9, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, boxShadow: '0 24px 60px rgba(31,57,101,0.25)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0F172A', margin: 0 }}>🏃 Create Sprint</h2>
              <button onClick={() => setShowCreateSprint(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569' }}>
                <X style={{ width: 20, height: 20 }} />
              </button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, display: 'block', textTransform: 'uppercase' }}>Project *</label>
                <select value={sprintForm.project_id} onChange={e => setSprintForm(f => ({ ...f, project_id: e.target.value }))}
                  style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc' }}>
                  <option value="">— Select a project —</option>
                  {projects.map(p => <option key={p.id} value={p.id}>{p.icon} {p.name}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, display: 'block', textTransform: 'uppercase' }}>Sprint Name *</label>
                <input value={sprintForm.name} onChange={e => setSprintForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Sprint 1 · v2.4 Release"
                  style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, display: 'block', textTransform: 'uppercase' }}>Sprint Goal</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, display: 'block', textTransform: 'uppercase' }}>Start Date</label>
                  <input type="date" value={sprintForm.start_date} onChange={e => setSprintForm(f => ({ ...f, start_date: e.target.value }))}
                    style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 4, display: 'block', textTransform: 'uppercase' }}>End Date</label>
              ...
              <button onClick={saveSprint} disabled={saving || !sprintForm.name.trim() || !sprintForm.project_id}
                style={{ padding: '12px', background: saving || !sprintForm.name.trim() || !sprintForm.project_id ? '#94A3B8' : '#6366f1', color: '#fff', border: 'none', borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
