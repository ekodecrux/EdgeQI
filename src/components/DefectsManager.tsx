...
export default function DefectsManager({ currentProjectId, currentSprintId, onNavigateTo }: Props) {
  ...
  const StatCard = ({ label, value, sub, color, icon: Icon }: any) => (
    <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #E2E8F0', flex: 1, minWidth: 120 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {Icon && <Icon style={{ width: 16, height: 16, color: color || '#475569' }} />}
      </div>
  ...
  return (
    <div style={{ padding: '0 0 40px', fontFamily: '"Inter", Arial, sans-serif' }}>

      {/* ─── Header ─── */}
      <div style={{ background: 'linear-gradient(135deg, #0F172A 0%, #2563eb 100%)', borderRadius: 16, padding: '24px 28px', marginBottom: 24, color: '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => setShowCreateModal(true)} style={{ background: '#fff', color: '#0F172A', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
          ...
          <StatCard label="Total" value={stats.total} sub="All time" color="#0F172A" icon={BarChart2} />
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          {(['list', 'kanban', 'analytics'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              style={{ padding: '5px 14px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700, textTransform: 'capitalize',
                background: view === v ? '#fff' : 'transparent', color: view === v ? '#0F172A' : '#475569',
        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search defects…"
            style={{ width: '100%', paddingLeft: 32, paddingRight: 12, paddingTop: 7, paddingBottom: 7, border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 12, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
        </div>

        {/* Status filter */}
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          style={{ padding: '7px 12px', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 12, color: '#0F172A', background: '#fff' }}>
        ...
        <button onClick={load} style={{ padding: '7px 12px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#fff', cursor: 'pointer' }}>
          <RefreshCw style={{ width: 14, height: 14, color: '#475569' }} />
        </button>
      </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
          {loading ? (
            <div style={{ padding: 48, textAlign: 'center' }}>
              <Loader2 style={{ width: 28, height: 28, color: '#5B6CFF', margin: '0 auto 12px', animation: 'spin 1s linear infinite' }} />
              <p style={{ color: '#475569', fontSize: 13 }}>Loading defects…</p>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: 60, textAlign: 'center' }}>
              <CheckCircle2 style={{ width: 40, height: 40, color: '#27ae60', margin: '0 auto 12px' }} />
              <p style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', marginBottom: 6 }}>No Defects Found</p>
              ...
              {!filterStatus && !filterSev && !search && (
                <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 20 }}>
                  <button onClick={() => setAnalyzeMode(true)} style={{ padding: '10px 20px', background: '#5B6CFF', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                    Analyze Test Failures
                  </button>
                  <button onClick={() => setShowCreateModal(true)} style={{ padding: '10px 20px', background: '#f8fafc', color: '#0F172A', border: '1px solid #E2E8F0', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                    Raise Manually
                  </button>
                </div>
              )}
              ...
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #E2E8F0' }}>
                  {['ID', 'Defect Title', 'Severity', 'Priority', 'Status', 'Module', 'Type', 'Raised', 'TMS', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((d, i) => (
                  <tr key={d.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafbfc' }}
                    ...
                    <td style={{ padding: '10px 12px', fontSize: 11, color: '#475569', fontFamily: 'JetBrains Mono, ui-monospace, monospace', whiteSpace: 'nowrap' }}>{d.id.slice(-8)}</td>
                    <td style={{ padding: '10px 12px', maxWidth: 280 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.title}</div>
                      {d.test_case_title && <div style={{ fontSize: 10, color: '#94A3B8' }}>📎 {d.test_case_title}</div>}
                      {d.raised_by === 'ai-auto' && <span style={{ fontSize: 9, background: '#eaf5fd', color: '#5B6CFF', padding: '1px 6px', borderRadius: 10, fontWeight: 700 }}>🤖 AI-Raised</span>}
                    </td>
                    <td style={{ padding: '10px 12px', whiteSpace: 'nowrap' }}><SevBadge s={d.severity} /></td>
                    <td style={{ padding: '10px 12px' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: d.priority === 'P0' ? '#c0392b' : d.priority === 'P1' ? '#e67e22' : '#475569' }}>{d.priority}</span>
                    </td>
                    ...
                    <td style={{ padding: '10px 12px', fontSize: 12, color: '#475569' }}>{d.module || '—'}</td>
                    <td style={{ padding: '10px 12px', fontSize: 11, color: '#475569', whiteSpace: 'nowrap' }}>{d.defect_type}</td>
                    <td style={{ padding: '10px 12px', fontSize: 11, color: '#94A3B8', whiteSpace: 'nowrap' }}>{new Date(d.raised_at).toLocaleDateString()}</td>
                    <td style={{ padding: '10px 12px' }}>
                      {d.tms_issue_key ? (
                        <a href={d.tms_url || '#'} target="_blank" rel="noreferrer"
                          style={{ fontSize: 11, color: '#5B6CFF', fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
                          {d.tms_issue_key} <ExternalLink style={{ width: 10, height: 10 }} />
                        </a>
                      ) : (
                        <button onClick={() => pushToTMS(d)} disabled={pushingId === d.id}
                          style={{ fontSize: 10, background: '#f0f7ff', color: '#5B6CFF', border: '1px solid #bee3f8', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontWeight: 700, whiteSpace: 'nowrap' }}>
                          {pushingId === d.id ? '…' : '↑ Jira'}
                        </button>
                      )}
                    </td>
                    <td style={{ padding: '10px 12px', whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button onClick={() => setSelectedDefect(d)} title="View details" style={{ padding: 5, background: '#f8fafc', border: '1px solid #E2E8F0', borderRadius: 6, cursor: 'pointer' }}>
      ...
      {view === 'kanban' && (
        <div style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 8 }}>
          {['Open', 'In Progress', 'Resolved', 'Closed'].map(col => {
            ...
            return (
              <div key={col} style={{ minWidth: 260, flex: '0 0 260px', background: '#f8fafc', borderRadius: 12, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', background: colColor.bg, borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: '70vh', overflowY: 'auto' }}>
                  {colDefects.map(d => (
                    <div key={d.id} onClick={() => setSelectedDefect(d)}
                      style={{ background: '#fff', borderRadius: 10, padding: 12, border: '1px solid #E2E8F0', cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                      onMouseEnter={e => (e.currentTarget.style.borderColor = '#5B6CFF')}
                      onMouseLeave={e => (e.currentTarget.style.borderColor = '#E2E8F0')}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#0F172A', marginBottom: 6, lineHeight: 1.3 }}>{d.title}</div>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <SevBadge s={d.severity} />
                        <span style={{ fontSize: 10, color: '#475569', background: '#f8fafc', border: '1px solid #E2E8F0', borderRadius: 10, padding: '1px 6px' }}>{d.module || 'General'}</span>
                        {d.raised_by === 'ai-auto' && <span style={{ fontSize: 9, background: '#eaf5fd', color: '#5B6CFF', padding: '1px 6px', borderRadius: 10, fontWeight: 700 }}>🤖 AI</span>}
                      </div>
                      {d.tms_issue_key && <div style={{ marginTop: 6, fontSize: 10, color: '#5B6CFF', fontWeight: 700 }}>🔗 {d.tms_issue_key}</div>}
                    </div>
                  ))}
                  {colDefects.length === 0 && <div style={{ padding: '20px 0', textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>No defects</div>}
                </div>
              </div>
            );
          })}
      ...
      {view === 'analytics' && stats && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: 20 }}>
            <h4 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 800, color: '#0F172A' }}>🔥 Defects by Module</h4>
            {(stats.byModule || []).map((m: any) => (
              <div key={m.module} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <span style={{ fontSize: 12, color: '#0F172A', fontWeight: 600 }}>{m.module || 'Unknown'}</span>
            ...
            {(!stats.byModule?.length) && <p style={{ color: '#94A3B8', fontSize: 12 }}>No data yet</p>}
          </div>

          {/* By Defect Type */}
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: 20 }}>
            <h4 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 800, color: '#0F172A' }}>📋 Defects by Type</h4>
            {(stats.byType || []).map((t: any) => (
              <div key={t.defect_type} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid #f1f5f9' }}>
                <span style={{ fontSize: 12, color: '#0F172A' }}>{t.defect_type}</span>
                <span style={{ fontSize: 13, fontWeight: 800, color: '#0F172A' }}>{t.c}</span>
              </div>
            ))}
          </div>

          {/* Trend */}
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: 20, gridColumn: '1/-1' }}>
            <h4 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 800, color: '#0F172A' }}>📈 Defects Raised — Last 14 Days</h4>
                ...
                {(stats.trend || []).map((t: any) => {
                  ...
                  return (
                    <div key={t.day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      ...
                      <span style={{ fontSize: 9, color: '#94A3B8', transform: 'rotate(-45deg)', transformOrigin: 'right', whiteSpace: 'nowrap' }}>{t.day?.slice(5)}</span>
                    </div>
                  );
                })}
              </div>
            ) : <p style={{ color: '#94A3B8', fontSize: 12 }}>No trend data yet. Defects raised over time will appear here.</p>}
          </div>
        </div>
      )}
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: '#f8fafc' }}>
              <div style={{ flex: 1, paddingRight: 16 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: 10, color: '#475569', fontFamily: 'JetBrains Mono, ui-monospace, monospace', background: '#f1f5f9', padding: '2px 6px', borderRadius: 4 }}>{selectedDefect.id}</span>
                </div>
                <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0F172A', margin: 0, lineHeight: 1.3 }}>{selectedDefect.title}</h3>
              </div>
              <button onClick={() => setSelectedDefect(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', padding: 4 }}><X style={{ width: 20, height: 20 }} /></button>
            </div>
            ...
            <div style={{ padding: 24, flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                {['Open','In Progress','Resolved','Closed'].map(s => (
                  <button key={s} onClick={() => { quickUpdate(selectedDefect.id, 'status', s); setSelectedDefect({...selectedDefect, status: s as any}); }}
                    style={{ fontSize: 11, padding: '5px 12px', borderRadius: 20, border: `1px solid ${STATUS_COLOR[s]?.text || '#E2E8F0'}`, background: selectedDefect.status === s ? (STATUS_COLOR[s]?.bg || '#f8fafc') : '#fff', color: STATUS_COLOR[s]?.text || '#475569', cursor: 'pointer', fontWeight: 700 }}>
                    {s}
                  </button>
                ))}
                <button onClick={() => pushToTMS(selectedDefect)} disabled={!!selectedDefect.tms_issue_key || pushingId === selectedDefect.id}
                  style={{ fontSize: 11, padding: '5px 14px', borderRadius: 20, border: '1px solid #5B6CFF', background: selectedDefect.tms_issue_key ? '#eaf5fd' : '#fff', color: '#5B6CFF', cursor: 'pointer', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {[
                ...
                <div key={f.label} style={{ display: 'flex', gap: 12, marginBottom: 10, borderBottom: '1px solid #f1f5f9', paddingBottom: 10 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#475569', minWidth: 130, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{f.label}</span>
                  <span style={{ fontSize: 13, color: '#0F172A', flex: 1 }}>{f.value}</span>
                </div>
              ) : null)}

              {selectedDefect.description && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>Description</div>
                  <div style={{ fontSize: 13, color: '#0F172A', background: '#f8fafc', borderRadius: 8, padding: 12, lineHeight: 1.6 }}>{selectedDefect.description}</div>
                </div>
              )}

              {selectedDefect.root_cause && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>Root Cause</div>
                  <div style={{ fontSize: 13, color: '#0F172A', background: '#fff5ec', borderRadius: 8, padding: 12, lineHeight: 1.6, borderLeft: '3px solid #e67e22' }}>{selectedDefect.root_cause}</div>
                </div>
              )}

              {selectedDefect.fix_suggestion && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>Fix Suggestion</div>
                  <div style={{ fontSize: 13, color: '#0F172A', background: '#f0fff4', borderRadius: 8, padding: 12, lineHeight: 1.6, borderLeft: '3px solid #27ae60' }}>{selectedDefect.fix_suggestion}</div>
                </div>
              )}

              {selectedDefect.failure_log && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>Failure Log</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 640, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0F172A', margin: 0 }}>🤖 AI Failure Analysis</h3>
                <p style={{ fontSize: 12, color: '#475569', margin: '4px 0 0' }}>Paste failure logs — AI decides which are real defects vs. script errors, then auto-raises defects</p>
              </div>
              <button onClick={() => { setAnalyzeMode(false); setAnalysisResult(null); setPastedLogs(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569' }}><X style={{ width: 20, height: 20 }} /></button>
            </div>

            <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>Paste failure logs / test output</label>
            <textarea value={pastedLogs} onChange={e => setPastedLogs(e.target.value)} rows={8}
              ...
              style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '10px 12px', fontSize: 12, color: '#0F172A', background: '#f8fafc', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'JetBrains Mono, ui-monospace, monospace', lineHeight: 1.5 }} />

            <button onClick={analyzeFromLogs} disabled={analyzeLoading || !pastedLogs.trim()}
              style={{ marginTop: 14, width: '100%', padding: '12px', background: analyzeLoading || !pastedLogs.trim() ? '#94A3B8' : 'linear-gradient(135deg,#0F172A,#5B6CFF)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                ...
                {analysisResult.error ? (
                  ...
                  <>
                    ...
                    {(analysisResult.analyses || []).map((a: any, i: number) => (
                      <div key={i} style={{ background: '#fff', borderRadius: 8, padding: 12, marginBottom: 8, border: '1px solid #E2E8F0' }}>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 10, color: '#475569' }}>Confidence: {a.confidence}%</span>
                        </div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 4 }}>{a.defect_title}</div>
                        <div style={{ fontSize: 11, color: '#475569' }}>{a.root_cause}</div>
      ...
      {showRegression && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0F172A', margin: 0 }}>🎯 Smart Regression Selection</h3>
                <p style={{ fontSize: 12, color: '#475569', margin: '4px 0 0' }}>Describe what changed — AI identifies only the impacted tests (60–80% reduction)</p>
              </div>
              <button onClick={() => { setShowRegression(false); setRegressionResult(null); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569' }}><X style={{ width: 20, height: 20 }} /></button>
            </div>

            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>What changed? *</label>
              <textarea value={changeDesc} onChange={e => setChangeDesc(e.target.value)} rows={3}
                ...
                style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '10px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', resize: 'vertical', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>Modules changed (comma-separated)</label>
              <input value={changedModules} onChange={e => setChangedModules(e.target.value)} placeholder="e.g. Payment, Login, Checkout"
                style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
            </div>

            <button onClick={runSmartRegression} disabled={regressionLoading || !changeDesc.trim()}
              style={{ width: '100%', padding: '12px', background: regressionLoading || !changeDesc.trim() ? '#94A3B8' : 'linear-gradient(135deg,#6366f1,#5B6CFF)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            {regressionResult && !regressionResult.error && (
              <div style={{ marginTop: 20 }}>
                <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                  {[
                    { label: 'Total Tests', value: regressionResult.total, color: '#475569' },
                    ...
                    <div key={c.label} style={{ flex: 1, minWidth: 100, background: '#f8fafc', borderRadius: 10, padding: '12px 16px', border: '1px solid #E2E8F0', textAlign: 'center' }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: c.color }}>{c.value}</div>
                      <div style={{ fontSize: 10, color: '#475569', fontWeight: 700, textTransform: 'uppercase' }}>{c.label}</div>
                    </div>
                  ))}
                </div>

                {regressionResult.rationale && (
                  <div style={{ background: '#eaf5fd', borderRadius: 8, padding: 12, marginBottom: 12, borderLeft: '3px solid #5B6CFF' }}>
                    <p style={{ fontSize: 12, color: '#0F172A', margin: 0, lineHeight: 1.5 }}>{regressionResult.rationale}</p>
                  </div>
                )}

                <h5 style={{ fontSize: 12, fontWeight: 800, color: '#0F172A', marginBottom: 8 }}>
                  Tests to include in regression suite ({regressionResult.impacted_count}):
                </h5>
                <div style={{ maxHeight: 220, overflowY: 'auto', background: '#f8fafc', borderRadius: 8, border: '1px solid #E2E8F0' }}>
                  {(regressionResult.impacted_tests || []).map((tc: any) => (
                    <div key={tc.id} style={{ padding: '8px 12px', borderBottom: '1px solid #E2E8F0', display: 'flex', gap: 10, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 12, color: '#0F172A', flex: 1 }}>{tc.title}</span>
                      <span style={{ fontSize: 10, color: '#94A3B8' }}>{tc.module}</span>
                    </div>
                  ))}
                  {!regressionResult.impacted_tests?.length && <div style={{ padding: 20, textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>No tests impacted</div>}
                </div>
              </div>
            )}
      ...
      {showCreateModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 580, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0F172A', margin: 0 }}>🐛 Raise New Defect</h3>
              <button onClick={() => { setShowCreateModal(false); setForm(emptyForm); setSaveErr(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569' }}><X style={{ width: 20, height: 20 }} /></button>
            </div>

            {/* Title */}
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Defect Title *</label>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <select value={form[f.key]} onChange={e => setForm((prev: any) => ({ ...prev, [f.key]: e.target.value }))}
                    style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 10px', fontSize: 13, color: '#0F172A', background: '#fff' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Module / Feature Area</label>
              <input value={form.module} onChange={e => setForm((f: any) => ({ ...f, module: e.target.value }))} placeholder="e.g. Login, Payment, Dashboard"
                style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Description</label>
              <textarea value={form.description} onChange={e => setForm((f: any) => ({ ...f, description: e.target.value }))} rows={3}
                ...
                style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', resize: 'vertical', boxSizing: 'border-box' }} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Root Cause</label>
                <input value={form.root_cause} onChange={e => setForm((f: any) => ({ ...f, root_cause: e.target.value }))} placeholder="Known root cause"
                  style={{ width: '100%', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#0F172A', background: '#f8fafc', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Assigned To</label>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={createDefect} disabled={saving || !form.title.trim()}
                style={{ flex: 1, padding: '12px', background: saving || !form.title.trim() ? '#94A3B8' : '#5B6CFF', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                {saving ? '⏳ Saving…' : '🐛 Raise Defect'}
              </button>
              <button onClick={() => { setShowCreateModal(false); setForm(emptyForm); setSaveErr(''); }}
                style={{ padding: '12px 20px', background: '#f8fafc', color: '#475569', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
