...
export const TmsSyncBar: React.FC<TmsSyncBarProps> = ({
  ...
  const handlePull = async () => {
    ...
    try {
      ...
      const savedMsg = d.saved > 0 ? ` · ${d.saved} saved to EdgeQI` : (d.saved === 0 ? ' · already in EdgeQI' : '');
      setStatus({ type: 'success', msg: `✅ Pulled ${items.length} ${MODULE_LABELS[module]} from ${cfg.tool.toUpperCase()}${savedMsg}${d.demo ? ' (demo data)' : ''}` });
      onPull?.(items);
    } catch (e: any) {
  ...
  // Not configured — show a visible amber warning banner
  if (!cfg) {
    if (compact) {
      return (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-amber-300 bg-amber-50">
          <Settings className="w-3 h-3 text-amber-500 shrink-0" />
          <span className="text-[10px] font-mono text-amber-700">No TMS —</span>
          <a href="#" onClick={(e) => { e.preventDefault(); window.dispatchEvent(new CustomEvent('navigate-tab', { detail: 'settings' })); }}
            className="text-[10px] font-mono font-bold text-blue-600 hover:underline">Connect →</a>
        </div>
      );
    }
    return (
      <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl border border-amber-300 bg-amber-50">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-bold text-amber-800 font-mono">No Test Management System connected</p>
            <p className="text-[10px] text-amber-600 font-mono mt-0.5">Connect Jira, Xray, Zephyr, TestRail, Azure DevOps, qTest or HP ALM to pull &amp; push {MODULE_LABELS[module]} in real time.</p>
          </div>
        </div>
        <a href="#" onClick={(e) => { e.preventDefault(); window.dispatchEvent(new CustomEvent('navigate-tab', { detail: 'settings' })); }}
          className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-amber-300 text-amber-700 text-[11px] font-mono font-bold hover:bg-amber-100 transition-all">
          <Settings className="w-3 h-3" /> Configure TMS →
        </a>
      </div>
    );
  }
